...
<script>
    $(document).on('click','.btn-field-remove',function(e){
        ...
        $.ajax({
            type: "POST",
            url: route,
            data: {FieldsId:remove_items,_token:'{{csrf_token()}}'},
    ...
    $(document).on("click", ".settingitem", function(e) {
        ...
        $.ajax({
            type: "POST",
            url: route,
            data: {fieldId:FieldId,update:update,value:value,_token:'{{csrf_token()}}'},
            success: function(data) {
            if(data.whateupdate == 'EmptyOrNot'){
                ...
                if(data.ProjectField.EmptyOrNot == 1 ){
                    ...
                    if(data.ProjectField.EmptyValue == 'null' && data.ProjectField.EmptyOrNot == 0){
                        printEmptyOrNotInhtml = `{{'Not:'.$project->name}}`
            ...
            $(`.emptyBoxinTr_${data.ProjectField.id}`).html(printEmptyOrNotInhtml)
                ...
                // printEmptyOrNot=`<div class="row col-12">
                //         ${printEmptyOrNot}
                //         <div  style="margin: 0; "class="dropdown-menu  col-12 form-group drop_set_style" aria-labelledby="task_helper_button_creat_{{$project->id}}_">
                //             <div class="row" style="padding-right: 15px;">
                //                 <div  class="dropdown-item settingitem col-12"  data-empty = '1'  data-field-id = '${FieldProjectid}' onclick="selectsetting($(this),{{$project->id}},'#ffffff','N')">
                //                     Can be empty
                //                 </div>
                //             </div>
                //             <div class="row" style="padding-right: 15px;">
                //                 <div  class="dropdown-item settingitem col-12"  data-empty = '0'  data-field-id = '${FieldProjectid}' onclick="selectsetting($(this),{{$project->id}},'#ffffff','N')">
                //                     Cannot be empty
                //                 </div>
                //             </div>
                //         </div>
    ...
    $(document).on('click','.FiledItem',function(e){
        ...
        $.ajax({
            type: 'POST', 
                url : route, 
                dataType: "json",
                data: {make:make,projectFieldId:FieldProjectid,NewEditItem:NewEditItem,_token:'{{csrf_token()}}'},
                beforeSend:function(){
                    $('#test-btn').remove()
            ...
            success: function(data) {
                ...
                if(data.field){
                    if(JSON.parse(data.field.properties).length>0){
                        item =`<button class=" btn-style btn  dropdown-toggle col-12 form-control default_button_update_${data.field.id}" id='task_helper_button_creat_{{$project->id}}_${data.field.id}'   type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    ${JSON.parse(data.field.properties)[0].name}
                                </button>
                                <div style="margin: 0; "class="dropdown-menu  col-12 form-group drop_set_style default_value${data.ProjectField.id}" aria-labelledby="task_helper_button_creat_{{$project->id}}_${data.field.id}">
                        ...
                        item = `<button class=" btn-style btn  dropdown-toggle col-12 form-control default_button_update_${data.field.id}"   type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                ...
                                <div  style="margin: 0; "class="dropdown-menu  col-12 form-group drop_set_style default_value${data.ProjectField.id}" aria-labelledby="default_button_update_${data.field.id}">
                                    ...
                                    <div class="row" style="padding-right: 15px;">
                                        <div  class=" settingitem col-12"   onclick="selectsetting($(this),{{$project->id}},'${data.field.name}',NOValue')">
                                            <p  class="settingitem col-12" >
                                                No options found
                                            </p>
                                        </div>
...
<script>
    ...
    $(document).on('mousedown','.fild_true',function (){
        ...
        if(AddRemove){
            ...
            $.ajax({
                ...
                url:   route,
                dataType: "json",
                data: {id:id,creat:creat,SettingId:SettingId,ProjectFieldId:FieldProjectId,_token:'{{csrf_token()}}'},
                success:function(data) {
                    console.log(data)
                    if(data.work == 'search'){
    ...
    $(document).on('click','.close',function(){
        ...
        $(`#NameOfSetting${open}`).val('')
        $(`#ColorOfSetting${open}`).val('')
    })
    $(document).on("click", ".SaveSettingInField", function(e) {
        ...
        let Creat = $(`#FieldCreatTrue${open}`).data('fieldCreat');
        let Fieldid = $(`#FieldId${open}`).data('fieldId');
        let NameOfSetting = $(`#NameOfSetting${open}`).val();
        let Description = $(`#DescriptionOfSetting${open}`).val();
        let ColorOfSetting = $(`#ColorOfSetting${open}`).val();
        let FieldProjectId = $(`#FieldProjectId`).data('fieldProjectId');
        let sendByjquery = true;
        let res = $('#FiledNameError');
        ...
        $.ajax({
            type: "POST",
            url: route,
            data: {sendByjquery:sendByjquery,creat:Creat,json_color:ColorOfSetting,Fieldid:Fieldid,json_name:NameOfSetting,description:Description,FieldProjectId:FieldProjectId,_token:'{{csrf_token()}}'},
            success: function(data) {
                console.log(data)
                if(data.error){
                    res.css('color', 'red').css('font-weight','Bold');
                    ...
                    $('.SaveSettingInField').removeAttr('disabled','disabled')
                    $(`#NameOfSetting${open}`).val('');
                    $(`#DescriptionOfSetting${open}`).val('');
                    $(`#ColorOfSetting${open}`).val('#ffffff');
                    $(`#addMember${open}`).modal('toggle');
                }
            },
            error: function(danger) {
                alert('error')
    ...
    $(document).on("click", ".value_edit", function(e) {
        ...
        if($(this).data('value') == 'edit'){
            ...
            console.log(`.value_edit_${FieldId}_${item}`)
            $(`#AddValue${FieldProjectid}`).click()
            $(`#NameOfSetting${open}`).val(`${json_name}`)
            $(`#ColorOfSetting${open}`).val(`${json_color}`)
            ...
            $.ajax({
                type: "POST",
                url: route,
                data: {id:item,sendByjquery:sendByjquery,_token:'{{csrf_token()}}'},
                success: function(data) {
                    console.log(data)
    ...
    $(document).on('click','.EditSettingInField',function(e){
        ...
        $.ajax({
            ...
            url: route,
            cache: false,
            data: {id:item,sendByjquery:sendByjquery,json_color:ColorOfSetting,json_name:NameOfSetting,description:Description,_token:'{{csrf_token()}}'},
            success: function(data) {
                isPost = 0;
                if(data.error){
                    res.css('color', 'red').css('font-weight','Bold');
        ...
        function NewValue(data){
            ...
            for(let i=0;i<JSON.parse(data.field.properties).length;i++){
                ...
                $(`#FieldId${open}`).data('fieldId',data.field.id);
