                    ...
                    <div class="col-lg-2 col-md-4 col-sm-5 col-5">
                        <select class="selectpicker" id="newSettingBtn" data-size="4" data-style="btn btn-primary btn-round everythingBtn" >
                            ...
                            @if($setting->Default == 1)
                                <option class="{{strtoupper($setting->name)}} setting_item" data-setting-id="{{ $setting->id }}" value="{{ $setting->id }}" >{{ $setting->name }}</option>
                            @endif
                            @endforeach
                        </select>
                                    ...
                                    <div class="row">
                                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                            <div class="form-group">
                                                ...
                                                <input type="hidden" class="form-control" id="setting_id" name="Fieldid" required>
                                            </div>
                                        </div>
                                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                            <div class="form-group">
...
@section('scripts')
    ...
    <script src="{{ asset('invoices/admin/js/colorpicker.js') }}"></script>
        <script>
        $(document).on("change","#newSettingBtn",function(){
            ...
            if($(this).val() == "0" ){
                ...
                $('#setting_name').val($('.filter-option-inner-inner').text())
                $('#setting_id').val($(this).val())
    ...
    <script>
        $("#setting_name").on('input',function(){
                ...
                if($(`.${$(this).val().toUpperCase()}`).length > 0){
                    $('#setting_id').val($(`.${$(this).val().toUpperCase()}`).val())
                    $("#new_or_note").val('false')
                }else{
                    $('#setting_id').val('')
                    $("#new_or_note").val('true')
