            ...
            <thead class="thead-light col-12 row">
                <tr class="col-12 row">
                    ...
                    <th scope="col" width="150" class="col-1 d-flex justify-content-end ">
                        ...
                        <div class="card-icon add removeBtnUser " style="pointer-events: none;" id="removeBtnUser"  data-toggle="modal" data-target="#confirmDelete" data-title="Delete User" data-message="Are you sure you want to delete this user ?">
                        ...
                        <div class="row col-12 task_title">
                            <div scope="col-1">
                                ...
                                @forelse($this_settings as $this_setting)
                                            @if($this_setting->setting_id == $this_settings[0]->setting_id && $this_setting->setting !== NULL)
                                    ...
                                    @if($task->user_id == \App\Helper\AuthHelper::user()->id)
                                        <div id="task_{{$task->id}}_check" class="task_check hidden " >
                                            <input type="checkbox" value="{{$task->id}}" data-name="{{$task->task_number}}" class="img_content" id="task_{{$task->id}}_checkbox">
                                        </div>
                                    @endif
                                            ...
                                            @if(count($task->taskUsers) > 0)
                                                <button  class="btn btn-secondary dropdown-toggle btn-style  col-12 form-control top-0"  type="button" id="dropdownUser{{$users[0]->name}}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                    @foreach($task->taskUsers as $is_it)
                                                        ...
                                                        <span class="{{str_replace(' ', '_', $is_it->users->name)}}">
                                                        {{$is_it->users->name}},
                                                        </span>
                                                        @endif
                                                    @endforeach
